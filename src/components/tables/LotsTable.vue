<template>
  <div class="flex flex-col justify-between h-full">
    <div>
      ...
      <av-table :headers="['Numero de lote', 'Edad (Semanas)', 'Numero de Gallinas', 'Fecha de creación', '']">
        <tr v-for="(lot, key) in lotsByPage" :key="key" class="border-b border-gray-200 hover:bg-gray-100">
          ...
          <td class="py-3 px-6 text-center">
            <div class="flex item-center justify-center gap-2">
              <button @click="router.push({name: 'showLots', params: {id: lot.id}})" class="w-4 mr-2 transform hover:scale-125 cursor-pointer">
                <fai class="text-av-100 hover:text-av-300" size="lg" icon="eye"/>
              </button>
              <button type="button" @click="openDeleteModal(lot)"  class="w-4 mr-2 transform hover:scale-110 cursor-pointer">
